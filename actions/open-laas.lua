...
local getIndexChoices = function(indexes)
  return hs.fnutils.map(
      indexes, function(index) return { ['text'] = index.attributes.title, ['id'] = libs.urlencode(index.id) } end
  )
end

function module.run()
  libs.showDailog(
      getIndexChoices(getIndexes()), function(choice)
        hs.urlevent.openURL(
            'https://laas-kibana.service.emarsys.net/app/kibana#/discover?_g=()&_a=(columns:!(_source),index:' ..
                choice.id .. ',interval:auto,query:(match_all:()),sort:!(\'@timestamp\',desc))'
        )
      end
  )
end
